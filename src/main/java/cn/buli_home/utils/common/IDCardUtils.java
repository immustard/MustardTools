...
public class IDCardUtils {
    ...
    public static String convert15To18(String idCard) {
        ...
        if (StringUtils.onlyRegex(idCard, RegexConstant.NUMBER)) {
            // 获取出生年月日
            String birthday = idCard.substring(6, 12);
            Date birthDate = DateUtils.parse(birthday, "yyMMdd");
            // 获取出生年(完全表现形式,如：2010)
            int sYear = NumberUtils.parseInt(DateUtils.format(birthDate, "yyyy"));
            if (sYear > 2000) {
            ...
            // 获取校验位
            char sVal = p_getCheckCode18(idCard18.toString());
    ...
    public static String convert18To15(String idCard) {
        if (!StringUtils.isEmpty(idCard) && isValidCard18(idCard)) {
    ...
    public static boolean isValidCard18(String idCard, boolean ignoreCase) {
        ...
        if (StringUtils.onlyRegex(code17, RegexConstant.NUMBER)) {
            // 获取校验位
            char val = p_getCheckCode18(code17);
            // 第18位
            return ignoreCase
                    ? Character.toLowerCase(val) == Character.toLowerCase(idCard.charAt(17))
                    : val == idCard.charAt(17);
        }
        return false;
    }
    ...
    public static Date getBirthDate(String idCard) {
        ...
        return StringUtils.isEmpty(birthByIdCard)
                ? null
                : DateUtils.parse(birthByIdCard, "yyyyMMdd");
    }
    ...
    private static char p_getCheckCode18(String code17) {
        int sum = p_getPowerSum(code17.toCharArray());
        return p_getCheckCode18(sum);
    }
