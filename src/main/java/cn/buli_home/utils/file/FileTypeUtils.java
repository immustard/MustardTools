...
public class FileTypeUtils {
    ...
    public static String getType(String fileStreamHexHead) {
        if (!MapUtils.isEmpty(FILE_TYPE_MAP)) {
            for (final Map.Entry<String, String> fileTypeEntry : FILE_TYPE_MAP.entrySet()) {
    ...
    public static String getType(InputStream in, int fileHeadSize) throws RuntimeException {
        return getType((IOUtils.readHex(in, fileHeadSize, false)));
    }
     ...
     * @throws RuntimeException 读取流引起的异常
     */
    public static String getType(InputStream in, boolean isExact) throws RuntimeException {
        ...
        return isExact
                ? getType(IOUtils.readHex8192Upper(in))
                : getType(IOUtils.readHex64Upper(in));
    }
    ...
    public static String getType(InputStream in, String filename, boolean isExact) throws RuntimeException {
        ...
        if (null == typeName) {
            ...
            final String extName = FileUtils.extName(filename);
     ...
     * @param isExact 是否精确匹配，如果为false，使用前64个bytes匹配，如果为true，使用前8192bytes匹配
     * @return 类型，文件的扩展名，未找到为{@code null}
     * @throws RuntimeException 读取文件引起的异常
     */
    ...
    public static String getTypeByPath(String path, boolean isExact) throws RuntimeException {
        return getType(new File(path), isExact);
    }
